...
_create_doctor_clean_fixture() {
    ...
    # history entry providing GraphQL rejection (supports the no-go)
    {
        ...
        echo "scope: domain"
        echo "status: active"
        echo "behavior_effect: never_suggest"
...
_doctor_memory_struct_dir="$_CAIRN_TMPDIR/doctor_memory_struct_$$"
mkdir -p "$_doctor_memory_struct_dir"
_create_doctor_clean_fixture "$_doctor_memory_struct_dir"
{
    echo "type: rejection"
    echo "domain: api-layer"
    echo "decision_date: 2024-02"
    echo "recorded_date: 2024-02"
    echo "summary: SOAP rejected"
    echo "rejected: SOAP"
    echo "reason: Existing REST API is enough"
    echo "revisit_when: never"
} > "$_doctor_memory_struct_dir/.cairn/history/2024-02_missing-structured.md"

_memory_struct_json="$_CAIRN_TMPDIR/doctor_memory_struct_json.txt"
(cd "$_doctor_memory_struct_dir" && bash "$_CAIRN_BIN" doctor --json 2>/dev/null || true) > "$_memory_struct_json"
_assert_doctor_json_contract "memory loop: missing structured fields matches schema" "$_memory_struct_json"
_assert_doctor_json_expr "memory loop: missing structured fields signal" "$_memory_struct_json" "data.memory_loop.signals.includes('history-missing-structured-fields')"

_doctor_memory_inactive_dir="$_CAIRN_TMPDIR/doctor_memory_inactive_$$"
mkdir -p "$_doctor_memory_inactive_dir"
_create_doctor_clean_fixture "$_doctor_memory_inactive_dir"
sed -i.bak 's/^status: active$/status: superseded/' "$_doctor_memory_inactive_dir/.cairn/history/2024-01_graphql-spike-rejected.md"
rm -f "$_doctor_memory_inactive_dir/.cairn/history/2024-01_graphql-spike-rejected.md.bak"

_memory_inactive_json="$_CAIRN_TMPDIR/doctor_memory_inactive_json.txt"
(cd "$_doctor_memory_inactive_dir" && bash "$_CAIRN_BIN" doctor --json 2>/dev/null || true) > "$_memory_inactive_json"
_assert_doctor_json_contract "memory loop: inactive output projection matches schema" "$_memory_inactive_json"
_assert_doctor_json_expr "memory loop: inactive output projection signal" "$_memory_inactive_json" "data.memory_loop.signals.includes('inactive-history-in-output')"

_doctor_memory_lowconf_dir="$_CAIRN_TMPDIR/doctor_memory_lowconf_$$"
mkdir -p "$_doctor_memory_lowconf_dir"
_create_doctor_clean_fixture "$_doctor_memory_lowconf_dir"
sed -i.bak 's/^confidence: high$/confidence: low/' "$_doctor_memory_lowconf_dir/.cairn/history/2024-01_graphql-spike-rejected.md"
rm -f "$_doctor_memory_lowconf_dir/.cairn/history/2024-01_graphql-spike-rejected.md.bak"

_memory_lowconf_json="$_CAIRN_TMPDIR/doctor_memory_lowconf_json.txt"
(cd "$_doctor_memory_lowconf_dir" && bash "$_CAIRN_BIN" doctor --json 2>/dev/null || true) > "$_memory_lowconf_json"
_assert_doctor_json_contract "memory loop: low confidence output projection matches schema" "$_memory_lowconf_json"
_assert_doctor_json_expr "memory loop: low confidence output projection signal" "$_memory_lowconf_json" "data.memory_loop.signals.includes('low-confidence-in-output')"
