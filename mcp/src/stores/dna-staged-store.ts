...
export class DnaStagedStore {
    ...
    async loadAll(): Promise<DNAStagedEntry[]> {
        let entries: string[];
        try {
        ...
        const results: DNAStagedEntry[] = [];
        for (const file of entries.filter(f => f.endsWith(".yaml"))) {
            const raw = await readFile(join(this.dir, file), "utf-8");
            results.push(DNAStagedEntrySchema.parse(yamlParse(raw)));
        }
        return results;
    }

    async load(id: string): Promise<DNAStagedEntry | null> {
        try {
            const raw = await readFile(join(this.dir, `${id}.yaml`), "utf-8");
            return DNAStagedEntrySchema.parse(yamlParse(raw));
        } catch (err: unknown) {
    ...
    async findPending(): Promise<DNAStagedEntry[]> {
