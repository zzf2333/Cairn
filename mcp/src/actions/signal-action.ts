...
import { warnIfNoContext } from "../utils/session-guard.js";
...
export async function signalAction(ctx: CairnContext, args: SignalArgs): Promise<Record<string, unknown>> {
    const contextWarning = await warnIfNoContext(ctx.stateStore);

    const { signal_type: signalType, domain, details, evidence } = args;
    ...
    let challenges: Array<{ level: string; conflict_with: string; description: string; required_response?: string; archived?: boolean; trauma?: boolean }> = [];
