...
```
cairn init      # 初始化 .cairn/、刷新 .cairn/SKILL.md 与工具引导块
cairn doctor    # 只读健康检查：格式、stale domain、Skill 漂移、写回信号
```

**`cairn doctor` 包含 Stale Detection：**
...
domains
  ⚠  api-layer  stale: 3 new history entries since 2024-03 — update .cairn/domains/api-layer.md directly
  ✓  auth       stable, updated 2024-07
```

不需要后台 daemon，`cairn doctor` 是命令触发的一次性扫描。stale 判断规则：domain 文件的 `updated` 早于该 domain 最新 history 条目的 `recorded_date`。
...
cairn_write_history(entry)   # 写入新的 history 条目
cairn_doctor()               # 返回结构化健康检查结果
cairn_match(keywords, files?)# hooks / 文件路径匹配
```
