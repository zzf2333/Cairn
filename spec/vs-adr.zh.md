...
**2. 没有注入时机的概念。**
ADR 放在一个目录中，要么全部读取，要么全部忽略。没有机制说"始终读取这个"、"仅在规划 API 工作时读取这个"或"只有在用户询问认证历史时才获取这个"。在会话开始时读取 30 个 ADR 的 AI 将大部分上下文预算消耗在可能与当前任务无关的历史记录上。Cairn 的激活管线——始终开启（`output.md`）、按域按需（`domains/*.md`）和通过 `cairn_context()` 精确查询——是 ADR 没有建模的概念。
...
**4. 没有域范围的上下文分离。**
拥有 20 个 ADR 的团队在同一目录中按时间顺序混合了关于认证、API 设计、状态管理、部署和测试的决策。当 AI 规划 API 变更时，它没有有效的方法只提取与 API 相关的历史记录。Cairn 的 Skeleton 将域映射到所有权边界，`domains/*.md` 视图按域预先压缩演化历史，使 AI 能够在不读取完整档案的情况下获得恰好相关的上下文。

**5. `rejected` 字段在结构上缺失。**
ADR 有"考虑的替代方案"章节，但它是可选的、自由格式的，很少以 AI 使用的足够具体性编写。被拒路径——被评估并排除的方向——是防止 AI 重新提出已放弃选项的最重要输入。Cairn 将被拒路径视为关键数据：每个带有 `behavior_effect: avoid_suggestion` 的演化事件都记录了尝试过什么以及为什么被排除。
...
**自动信号捕获。**
Cairn 从三个来源自动捕获项目信号：Git 耳朵（revert、依赖变更、大规模重构）、对话耳朵（AI 通过 `cairn_signal()` 报告用户拒绝、决策、约束）和校准耳朵（检测代码现实与认知状态之间的漂移）。ADR 需要人类作者编写每条记录。Cairn 的三耳设计意味着约束相关事件在发生时即被捕获，无需手动文档步骤。

**基于 Gravity 的认知路由与人工审核。**
捕获的信号通过 Trust Router 分配引力（G0–G3）。G0 信号是噪声，直接丢弃。G1 建议在 lightweight 模式下自动写入，在 standard 模式下系统验证。G2 反思质询要求 AI 呈现推理。G3 硬约束始终需要人类批准。创伤域永久升级新信号的引力。DNA 特征基于项目人格调制引力。这确保了认知质量，同时不要求人类编写每条记录。

**Token 预算感知的约束视图。**
Cairn 从 Blood 事件自动生成 `views/output.md`，Token 预算目标 500，硬限制 800。格式使用 `key: value` 键值对和简短列表，而不是散文。每行必须能改变 AI 行为——如果删去一行不会改变回复，Views Engine 不会包含它。

**机器可读的 `behavior_effect` 字段。**
每个演化事件声明其 `behavior_effect` ——AI 行为应如何改变。支持四种类型：
...
**通过 Skeleton 实现域范围的上下文分离。**
Skeleton 中的每个域（`api-layer`、`auth`、`state-management` 等）定义了所有权边界和因果关键词。当 AI 调用 `cairn_context({ task: "API 设计" })` 时，激活引擎将任务映射到相关域，只返回域特定的约束——而不是认证历史，也不是部署历史。
...
|--|-----|--------------|
| 受众 | 人类工程师 | AI 认知系统 |
| 格式 | 散文段落 | 结构化 YAML（引力、生命周期、创伤） |
| 目的 | 制度记忆 | 行为约束输入 |
| 创建方式 | 人类编写 | 从信号捕获，经引力路由 |
| 注入 | 人类按需阅读 | 由 `cairn_context()` MCP 工具返回 |
| 约束 | 无——仅供参考 | `behavior_effect`、`lifecycle`、`trauma`、`governance` |
...
**Cairn 版本**（`.cairn/blood/evt_2023_03_database_postgresql.yaml`）：
...
Views Engine 自动生成 `views/output.md`（stack 章节含 `db: PostgreSQL`）和 `views/domains/database.md`（含演化轨迹）。

ADR 讲述了故事。Cairn 演化事件执行了约束。两者都是正确的，各自服务于其目的。两者互不替代。
