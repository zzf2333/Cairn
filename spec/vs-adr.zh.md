...
**自动信号捕获。**
Cairn 从两个来源自动捕获项目信号：Git 耳朵（revert、依赖变更、大规模重构）和对话耳朵（AI 通过 `cairn_signal()` 报告用户拒绝、决策、约束）。ADR 需要人类作者编写每条记录。Cairn 的双耳设计意味着约束相关事件在发生时即被捕获，无需手动文档步骤。
...
**Cairn 版本**（`.cairn/memory/mem_2023_03_database_postgresql.yaml`）：
