...
The behavior layer is the Skill adapter files — one per tool — that specify how each
tool should interact with the three layers. These files describe the injection timing
(always read `output.md`; read domain files when hooks match; query history on demand),
but the underlying data they read is always the same.

Skill adapter file locations:
...
| OpenCode | `AGENTS.md` (appended, shared with Codex) |

Adapter files are generated by `cairn init` for the tools the team selects. They can
be regenerated at any time without affecting the `.cairn/` data directory.

The behavioral contract across all adapters is the same: read `output.md` at the start
of every session; read the relevant domain file when the user's request matches a hook
keyword; query history entries when a precise lookup of a specific past decision is
needed. The format of the adapter file differs per tool; the semantics do not.
