...
| History 字段质量 | 每个 history 条目都必须包含非空 `rejected:` 字段，以及结构化记忆字段（`scope`、`status`、`behavior_effect`、`confidence`）。 |
...
| `history-missing-structured-fields` | 某个 history 条目缺少 `scope`、`status`、`behavior_effect` 或 `confidence` |
| `history-invalid-structured-field` | 某个结构化 history 字段的值不在允许枚举内 |
| `inactive-history-in-output` | `status: stale` 或 `status: superseded` 的 history 条目仍投影在 `output.md` 中 |
| `low-confidence-in-output` | `confidence: low` 的 history 条目出现在 `output.md` 中 |
