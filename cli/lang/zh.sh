...
msg_help_cmd_reflect()         { echo "反思近期工作，生成暂存更新候选条目"; }
msg_help_cmd_audit()           { echo "追踪迁移清理：开始审计或扫描残留"; }
...
msg_init_domains_dir_hint1()   { echo "  Domain 文件由 AI 使用原生文件工具（Write/Edit）直接写入"; }
msg_init_domains_dir_hint2()   { echo "  用于积累每个域的路径依赖约束"; }
msg_init_domains_dir_hint3()   { echo "  触发：output.md 的 hooks 节引用各域"; }
msg_init_domains_dir_hint4()   { echo "  更新：决策确定时 AI 直接更新域文件，无需命令"; }
...
msg_init_step_upgrade()        { echo "检查 v0.0.11 残留"; }
...
msg_init_next1()               { echo "  1. 在 AI 工具中安装 Cairn 引导块（如果上一步跳过了）"; }
msg_init_next2()               { echo "  2. 会话开始时，AI 自动读取 .cairn/output.md 和 .cairn/SKILL.md"; }
msg_init_next3()               { echo "  3. AI 直接写入 history / domain / output 文件 — 无需跑任何 CLI 命令"; }
msg_init_next4()               { echo "  4. 随时运行 'cairn doctor' 验证记忆健康状态"; }
...
# stage 候选类型路由提示（v0.0.8）
msg_stage_kind_history()        { echo "  [history-candidate] → 接受后将移动到 .cairn/history/"; }
msg_stage_kind_domain()         { echo "  [domain-update-candidate] → 接受后将在目标域文件上打开 \$EDITOR"; }
msg_stage_kind_output()         { echo "  [output-update-candidate] → 接受后将在 .cairn/output.md 上打开 \$EDITOR"; }
msg_stage_kind_audit()          { echo "  [audit-candidate] → 接受后将移动到 .cairn/audits/"; }
msg_stage_kind_legacy()         { echo "  [history-candidate（旧格式）] → 接受后将移动到 .cairn/history/"; }
msg_stage_accepted_audit()      { echo "✓  已接受 → audits/${1}"; }
msg_stage_open_editor_domain()  { echo "  正在打开 \$EDITOR 编辑 .cairn/domains/${1}.md — 请粘贴或合并候选内容"; }
msg_stage_open_editor_output()  { echo "  正在打开 \$EDITOR 编辑 .cairn/output.md — 请粘贴或合并候选内容"; }
msg_stage_editor_fallback()     { echo "  未设置 \$EDITOR — 候选条目保留在 staged/ 供手动审查"; }
msg_stage_audits_dir_created()  { echo "  已创建 .cairn/audits/"; }

# ── reflect 命令 ──────────────────────────────────────────────────────────────
msg_reflect_title()             { echo "Cairn Reflect — 任务后写回"; }
msg_reflect_no_git()            { echo "不是 git 仓库 — cairn reflect 需要 git 历史"; }
msg_reflect_no_cairn_warning()  { echo "未找到 .cairn/ 目录 — 请先运行 cairn init 初始化 Cairn"; }
msg_reflect_no_commits()        { echo "指定范围内未找到提交"; }
msg_reflect_scanning()          { echo "正在扫描近期变更..."; }
msg_reflect_git_range()         { echo "提交范围：${1}"; }
msg_reflect_diff_mode()         { echo "模式：已暂存差异（git diff --staged + 未暂存）"; }
msg_reflect_since_mode()        { echo "模式：自 ${1} 起的提交"; }
msg_reflect_commit_mode()       { echo "模式：自 ${1} 起的提交"; }
msg_reflect_commits_found()     { echo "范围内提交数：${1}"; }
msg_reflect_phase_reverts()     { echo "  revert 提交：${1} 个"; }
msg_reflect_phase_migrations()  { echo "  迁移关键词提交：${1} 个"; }
msg_reflect_phase_domains()     { echo "  涉及域文件的提交：${1} 个"; }
msg_reflect_phase_stack()       { echo "  stack 漂移信号：${1} 个"; }
msg_reflect_candidate_written() { echo "  ✓ ${1}  [${2}]"; }
msg_reflect_summary_header()    { echo "已生成候选条目："; }
...
msg_reflect_summary_total()     { echo "  合计：${1} 个候选条目已写入 .cairn/staged/"; }
msg_reflect_dry_run_total()     { echo "  合计：${1} 个候选条目将被写入（dry-run，未保存）"; }
...
# ── audit 命令 ────────────────────────────────────────────────────────────────
msg_audit_title()               { echo "Cairn Audit — 迁移清理追踪"; }
msg_audit_no_cairn_warning()    { echo "未找到 .cairn/ 目录 — 请先运行 cairn init 初始化 Cairn"; }
msg_audit_start_header()        { echo "正在为域 '${1}' 创建审计文件..."; }
msg_audit_start_written()       { echo "✓  已创建 .cairn/audits/${1}"; }
msg_audit_start_hint()          { echo "  编辑文件添加预期删除项，然后运行：cairn audit scan ${2}"; }
msg_audit_start_conflict()      { echo "冲突：.cairn/audits/${1} 已存在 — 请先重命名或删除"; }
msg_audit_start_domain_warn()   { echo "警告：域 '${1}' 不在锁定域列表中（继续执行）"; }
msg_audit_scan_header()         { echo "Cairn Audit Scan — 残留检测"; }
msg_audit_scan_scanning()       { echo "正在扫描残留..."; }
msg_audit_scan_domain_filter()  { echo "筛选域：${1}"; }
msg_audit_scan_no_audits()      { echo ".cairn/audits/ 中未找到审计文件 — 运行：cairn audit start <domain> --trigger \"...\""; }
msg_audit_scan_file()           { echo "  审计：${1}  (${2})"; }
msg_audit_scan_rejected_kw()    { echo "    正在检查 domains/${1}.md 中被拒路径的关键词..."; }
msg_audit_scan_hit()            { echo "    ${C_YELLOW}⚠${C_RESET}  ${1}：模式 '${2}' 匹配到 ${3}"; }
msg_audit_scan_no_hits()        { echo "    ${C_GREEN}✓${C_RESET}  此审计未发现残留"; }
msg_audit_scan_summary()        { echo "扫描完成：在 ${2} 个审计文件中发现 ${1} 处残留匹配"; }
msg_audit_scan_hint()           { echo "  用发现结果更新 .cairn/audits/ 文件，然后运行：cairn doctor"; }
msg_audit_missing_trigger()     { echo "错误：--trigger 为必填项 — 用法：cairn audit start <domain> --trigger \"<变更>\""; }
msg_audit_missing_domain()      { echo "错误：需要域名 — 用法：cairn audit start <domain> --trigger \"<变更>\""; }
msg_audit_unknown_sub()         { echo "未知子命令 '${1}' — 可用：cairn audit start | cairn audit scan"; }

msg_audit_help()                { cat <<'HELP'
用法：cairn audit <子命令> [选项]

显式追踪迁移清理义务。

子命令：
  start <domain> --trigger "<变更>"
      创建新的 .cairn/audits/YYYY-MM_<domain>-<slug>.md 审计文件。
      在迁移后运行以记录预期清理项。

  scan [<domain>]
      检测残留：扫描源文件中来自 rejected paths 的模式，
      并检查代码中是否仍然引用了已删除的依赖。
      可选：只扫描某个域。
...
# ── doctor 审计检查（v0.0.8）──────────────────────────────────────────────────
msg_doctor_section_audits()     { echo "── 审计"; }
msg_doctor_audit_ok()           { echo "无审计问题"; }
msg_doctor_audit_missing()      { echo "transition '${1}'（${2}）没有对应的审计文件 — 建议：cairn audit start"; }
msg_doctor_audit_stale()        { echo "审计 '${1}' 状态为 partial，已 ${2} 天 — 可能需要跟进"; }
msg_doctor_audit_complete()     { echo "✓  ${1} 已完成"; }
msg_doctor_stack_section()      { echo "── Stack 漂移"; }
...
msg_doctor_section_skill_guide()    { echo "── Cairn 引导块"; }
msg_doctor_skill_guide_ok()         { echo ".claude/CLAUDE.md 中的 Cairn 引导块已是最新"; }
msg_doctor_skill_guide_missing()    { echo ".claude/CLAUDE.md 中无 Cairn 引导块 — 请运行：cairn init"; }
msg_doctor_skill_guide_old()        { echo ".claude/CLAUDE.md 中的 Cairn 块为旧格式 — 请运行：cairn init --refresh-skills"; }

msg_doctor_section_skill_md()       { echo "── .cairn/SKILL.md"; }
msg_doctor_skill_md_ok()            { echo ".cairn/SKILL.md 已存在且为最新版本"; }
msg_doctor_skill_md_missing()       { echo "未找到 .cairn/SKILL.md — 请运行：cairn init"; }
msg_doctor_skill_md_stale()         { echo ".cairn/SKILL.md 与正版不一致 — 请运行：cairn init --upgrade"; }

msg_doctor_section_v0011()          { echo "── v0.0.11 残留"; }
msg_doctor_v0011_ok()               { echo "未发现 v0.0.11 残留"; }
msg_doctor_v0011_staged()           { echo ".cairn/staged/ 存在（${1} 个文件）— v0.0.12 不再使用此目录；请人工审阅后删除"; }
msg_doctor_v0011_audits()           { echo ".cairn/audits/ 存在（${1} 个文件）— 请将有用内容合并到域文件 ## known pitfalls 后删除"; }
msg_doctor_v0011_reflections()      { echo ".cairn/reflections/ 存在（${1} 个文件）— v0.0.12 不再生成此类文件；可安全删除"; }
