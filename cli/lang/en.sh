...
msg_init_domains_dir_hint1()   { echo "  Domain files are written by AI using its native file tools (Write/Edit)"; }
...
msg_init_domains_dir_hint4()   { echo "  Update: AI updates domain files directly when decisions crystallize"; }
...
msg_init_step_upgrade()        { echo "Check for v0.0.11 residue"; }
...
msg_init_next1()               { echo "  1. Install the Cairn guide block in your AI tool (if you skipped it above)"; }
msg_init_next2()               { echo "  2. At session start, AI reads .cairn/output.md and .cairn/SKILL.md automatically"; }
msg_init_next3()               { echo "  3. AI writes history / domain / output entries directly — no CLI needed"; }
msg_init_next4()               { echo "  4. Use 'cairn doctor' to verify memory health anytime"; }
...
msg_doctor_skill_guide_ok()         { echo "Cairn guide block is up to date in .claude/CLAUDE.md"; }
msg_doctor_skill_guide_missing()    { echo ".claude/CLAUDE.md has no Cairn guide block — run: cairn init"; }
msg_doctor_skill_guide_old()        { echo "Cairn block in .claude/CLAUDE.md appears to be old format — run: cairn init --refresh-skills"; }

msg_doctor_section_skill_md()       { echo "── .cairn/SKILL.md"; }
msg_doctor_skill_md_ok()            { echo ".cairn/SKILL.md is present and up to date"; }
msg_doctor_skill_md_missing()       { echo ".cairn/SKILL.md not found — run: cairn init"; }
msg_doctor_skill_md_stale()         { echo ".cairn/SKILL.md differs from canonical — run: cairn init --upgrade"; }

msg_doctor_section_v0011()          { echo "── v0.0.11 residue"; }
msg_doctor_v0011_ok()               { echo "No v0.0.11 residue found"; }
msg_doctor_v0011_staged()           { echo ".cairn/staged/ found (${1} file(s)) — no longer used in v0.0.12; review manually then delete"; }
msg_doctor_v0011_audits()           { echo ".cairn/audits/ found (${1} file(s)) — merge useful content into domain ## known pitfalls then delete"; }
msg_doctor_v0011_reflections()      { echo ".cairn/reflections/ found (${1} file(s)) — no longer generated; safe to delete"; }
