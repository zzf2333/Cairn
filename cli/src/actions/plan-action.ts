...
import { requireContext } from "../utils/session-guard.js";
...
export async function planAction(ctx: CairnContext, args: PlanArgs): Promise<Record<string, unknown>> {
    const guard = await requireContext(ctx.stateStore);
    if (!guard.ok) {
        throw new ContextNotLoadedError(guard.warning!);
    }
