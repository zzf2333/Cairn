...
Cairn 是一个 AI 维护的项目记忆引擎。它从 Git 历史和 AI 对话中自动捕获项目决策、被拒路径和
已接受的权衡取舍，然后通过信任系统路由到结构化记忆中，供 AI 编程助手作为行为约束消费。
...
Cairn 是一个**面向 AI 编程助手的动态记忆引擎**——它捕获、路由、存储和提供项目约束，
...
Cairn 从两个来源捕获项目信号，通过信任系统路由，并将结果作为结构化约束提供。
...
**Trust Router（L0–L3）：**
- **L0 丢弃** — 噪声或重复，直接丢弃
- **L1 候选** — 保存到 `signals/`，积累达阈值后升级 L2
- **L2 待审** — 等待人工通过 `cairn review` 审核
- **L3 自动写入** — 严格条件满足，自动写入 `memory/`

**Memory → Views → 约束：**
- `memory/` 是源数据（结构化 YAML，git diff 友好）
...
![Trust Router 流程图](docs/diagrams/06-trust-router-flow.png)

### 四种约束行为

每条记忆声明一个 `behavior_effect`：
...
交互流程：项目名称 → 开始日期 → 域选择 → Git 历史扫描 → 目录生成。
...
**Claude Code** — `~/.claude/mcp.json`（全局）或 `.claude/mcp.json`（项目）：
...
初始化和 MCP 配置完成后，日常操作完全自动：

1. **会话开始** — AI 调用 `cairn_context()` 加载约束
2. **工作过程中** — AI 检测到决策、否决或约束时调用 `cairn_signal()`
3. **会话结束** — AI 调用 `cairn_session_end()` 处理信号并重新生成视图

唯一需要人工操作的：定期运行 `cairn review` 审核待审条目。
...
不支持 MCP 的工具通过 Skill 适配文件直接读取 `views/`：
...
| `cairn doctor` | 健康诊断 |
| `cairn stage confirm` | 确认阶段建议 |
| `cairn memory show <id>` | 查看一条记忆 |
