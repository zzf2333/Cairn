...
<p>Cairn 是让这个生命体的认知跨越 AI 会话保持鲜活的认知运行时协议 — 以 MCP server + 行为契约的形式交付。</p>
...
**协议层** — 安装到 AI 指令文件中的行为规则（通过 `cairn skill install`），定义何时调用每个工具、如何处理约束、何时可以跳步。没有协议，AI 只能依赖 MCP 工具描述中的压缩版指令 — 足够初始化，但无法完整执行生命周期。

**工具层** — 协议调用的 16 个 MCP 工具：

| 工具 | AI 何时调用 | 干什么 |
|------|-----------|--------|
| `cairn_context` | 任务开始（session guard） | 创建活跃 session，激活当前域的认知、约束、挑战，检测 stale session |
| `cairn_plan` | 做设计 / 架构工作前 | 拉取本任务相关的历史约束 + DNA 指引（需先调用 `cairn_context`） |
| `cairn_signal` | 检测到决策 / 拒绝 / 约束时 | 经 TrustRouter 路由,记入治理日志 |
| `cairn_observe` | 每次 git commit 前 | 从近期工作中提取并路由候选信号 |
| `cairn_session_end` | 会话结束并带摘要 | 扫 git → 衰减 → 校准 → 阶段推断 → DNA 压缩 |
| `cairn_session_recover` | 检测到 stale session 时 | 对中断的 session 运行 session_end 管道 |
...
**每次会话必走**：`cairn_context`（开始）+ `cairn_session_end`（结束）。`cairn_context` 充当 session guard — 追踪活跃 session 状态机并检测中断 session。`cairn_plan` 未先调用 `cairn_context` 会拒绝执行；`cairn_signal` 会警告但继续。
...
协议告诉 AI *何时*调用每个工具 —— 完整的会话契约。将其安装到项目指令文件中：

| 宿主 | 命令 |
|------|------|
| Claude Code | `cairn skill install claude-code`（追加到 `CLAUDE.md`） |
| Codex | `cairn skill install codex`（追加到 `AGENTS.md`） |

或手动将 `skills/claude-code/SKILL.md` / `skills/codex.md` 追加到指令文件。
...
| `cairn skill install\|status\|update\|show` | 协议管理 | 安装或更新认知运行时协议 |
