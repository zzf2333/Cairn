...
**第二层（域文件）：** 带有 `hooks` 关键词列表的 YAML 前置元数据，用于意图检测，后跟 no-go 规则、已知陷阱和演进历史的 Markdown 章节。**整体覆盖式更新**，不追加——源事件保留在 `history/`。

**第三层（历史文件）：** 裸 `key: value` 格式（无 YAML fences）。每个文件是一个结构化源事件，包含 `scope`、`status`、`behavior_effect`、`confidence`，以及 `summary`、`rejected`、`chosen`、`reason`、`revisit_when` 等决策字段。`output.md` 与域文件是这一源层的运行时投影。`rejected` 字段仍然最关键——它记录了 AI 最可能重新提出的路径。
